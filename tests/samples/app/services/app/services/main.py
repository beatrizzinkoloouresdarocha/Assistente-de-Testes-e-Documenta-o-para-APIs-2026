...
@app.command()
def generate(
    target_file: str = typer.Argument(..., help="Caminho do arquivo da API (ex: tests/samples/sample_api.py)"),
    max_retries: int = typer.Option(3, help="Número máximo de tentativas de auto-correção")
):
    """
    ...
    if not target_path.exists():
        typer.secho(f"Erro: Arquivo '{target_file}' não encontrado.", fg=typer.colors.RED)
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if passed:
            typer.secho(f"✅ Sucesso! Todos os testes passaram e foram salvos em: {test_file_path}", fg=typer.colors.GREEN)
            return

        typer.secho(f"⚠️ Testes falharam! Enviando erro para a IA corrigir...", fg=typer.colors.YELLOW)
        test_code = fix_test_code(source_code, test_code, error_log)
        test_file_path.write_text(test_code, encoding="utf-8")

    typer.secho(f"❌ Não foi possível corrigir os testes após {max_retries} tentativas.", fg=typer.colors.RED)
